...
def dispatch(n):
    ...
    if diff > 0 or all([data["cpu"] == 0, data["normal"]>=10]):
        diff = max(diff, -2)
        cpu = diff + cpu
        normal = -diff + normal
...
def get_left_limit(up_limit=30):
    # 若限制总数
    # up_limit=40
    # res = cmd_popen("jjobs | wc -l")[0]
    # num = int(res)
    # left_num = up_limit + 1 - num
...
def sub_left_jobs(file="paths.temp", n=1, run_file=None):
    ...
    for i, node_marki in zip(to_word, node_mark):
        ...
        try:
            os.chdir(i)
            if run_file is None:
                with open(f"gjj_wang_{node_marki}.run", "w") as ff:
                    ff.write(get_node_run(run_file=None, name=node_marki))
                res = cmd_popen(f"jsub < $(find -name gjj_wang_{node_marki}.run)")[0]
            elif "#!/bin" in run_file:
                with open(f"gjj_wang_special.run", "w") as ff:
                    ff.write(get_node_run(run_file=run_file, name=node_marki))
                res = cmd_popen(f"jsub < $(find -name gjj_wang_special.run)")[0]
            else:
...
if __name__ == "__main__":
    ...
    # 0. file="paths.temp"为存放所有vasp路径名称的文件，每行为绝对路径.
